...
/obj/machinery/atmospherics/omni/filter/process()
	...
	if (transfer_moles > MINUMUM_MOLES_TO_FILTER)
		power_draw = filter_gas_multi(src, filtering_outputs, input_air, output_air, transfer_moles, power_rating)/2
...
/obj/machinery/atmospherics/omni/filter/proc/mode_send_switch(var/mode = ATM_NONE)
	switch(mode)
		...
		if(ATM_P)
			return "Toxins" //*cough* Plasma *cough*
		if(ATM_N2O)
...
/obj/machinery/atmospherics/omni/filter/Topic(href, href_list)
	...
	if(configuring && !use_power)
		switch(href_list["command"])
			...
			if("switch_filter")
				var/new_filter = input(usr,"Select filter mode:","Change filter",href_list["mode"]) in list("None", "Oxygen", "Nitrogen", "Carbon Dioxide", "Toxins", "Nitrous Oxide")
