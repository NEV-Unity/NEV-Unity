...
/obj/item/weapon/reagent_containers/glass
	...
	afterattack(obj/target, mob/user , flag)
		...
		if(ismob(target) && target.reagents && reagents.total_volume)
			...
			msg_admin_attack("[key_name_admin(user)] splashed [key_name_admin(M)] with [src.name]. Reagents: [contained] (INTENT: [uppertext(user.a_intent)]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
