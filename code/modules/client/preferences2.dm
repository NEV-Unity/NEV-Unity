...
datum/preferences
	...
	var/government_relation = "Neutral"
	...
	if(languages && languages.len)
		for(var/languages_name in languages)
			if(language_datums[languages_name])
				var/datum/language/L = language_datums[languages_name]
				total_cost += L.cost
				dat += "[languages_name] <a href='byond://?src=\ref[user];preference=speech;task=remove;language=[languages_name]'>\[remove\]</a><br>"
		dat += "<b>Used:</b> [total_cost] points.<br>"
	else
		dat += "no languages selected.<br>"

//point code

	if(total_cost < MAX_GEAR_COST)
		dat += " <a href='byond://?src=\ref[user];preference=loadout;task=input'>\[add gear\]</a><br>"
...
*/
				...
				if(C && total_cost <= MAX_GEAR_COST)
					gear += choice
					user << "\blue Added [choice] for [C.cost] points."
				else
...
*/
				...
				total_cost += C.cost
				if(C && total_cost <= MAX_GEAR_COST)
					if(C.name in languages)
						user << "\red That language is already known by this character."
					else
						languages += choice
						user << "\blue Added [choice] for [C.cost] points."
				else
	...
	switch(href_list["task"])
		...
		if("input")
			switch(href_list["preference"])
				...
				if("nt_relation")
					var/new_relation = input(user, "Choose your relation to the USN. Note that this represents what others can find out about your character by researching your background, not what your character actually thinks.", "Character Preference")  as null|anything in list("Loyal", "Supportive", "Neutral", "Skeptical", "Opposed")
					if(new_relation)
						government_relation = new_relation
...
/datum/preferences/proc/get_cost()
	var/total_cost = 0
